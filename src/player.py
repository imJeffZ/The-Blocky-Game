...
class RandomPlayer(Player):
    ...
    #     _level >= 0
    """A Random Player
    A random player chooses random moves and has unlimited smashes
    """
    _selected_block: Optional[Block]
    _level: int
...
class SmartPlayer(Player):
    ...
    #     _level >= 0
    """A Smart Player
    A smart player compares a series of random moves and selects the best one
    the number of moves to compare is determined by the difficulty
    a smart player cannot smash
    """
    _selected_block: Optional[Block]
    _level: int
    ...
    def __init__(self, renderer: Renderer, player_id: int,\
        ...
        self._selected_block = None
        if difficulty == 0:
            self._moves_to_compare = 5
        elif difficulty == 1:
            self._moves_to_compare = 10
        elif difficulty == 2:
            self._moves_to_compare = 25
        elif difficulty == 3:
            self._moves_to_compare = 50
        elif difficulty == 4:
            self._moves_to_compare = 100
        elif difficulty >= 5:
